...
- [How does StyleX compile inside TanStack Start on Vite, and is LightningCSS worth adding?](issues/02-stylex-tanstack.md) — `@stylexjs/unplugin` 0.19.0 first in the plugin list, CSS via a side-effect import in the root route; LightningCSS is already inside Vite 8 and the unplugin, so only a `browserslist` field and `lightningcssOptions.minify` are wired; base components must accept a `style` prop and spread `stylex.props()` themselves.
